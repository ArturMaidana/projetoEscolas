...
class UserController {
    ...
    async register(request, response) {
        ...
        try {
            const { nome, estado, cidade, email, password } = request.body;
            if (!nome || !estado || !cidade || !email || !password)
    ...
    async getProfile(request, response) {
        const httpHelper = new HttpHelper(response);
        try {
            // A informação do usuário está disponível no objeto de solicitação (request) devido ao middleware de autenticação
            console.log("User ID from middleware:", request.userId);

            // Buscar as informações do usuário no banco de dados usando o ID
    ...
    async delete(request, response) {
        ...
        try {
            ...
            if (!userAlreadyExists)
                return httpHelper.notFound("Usuario não encontrado!");
